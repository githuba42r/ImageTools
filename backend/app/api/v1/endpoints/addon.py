...
async def exchange_token(
    ...
    # Broadcast addon_connected event to all WebSocket clients for this user
...
async def upload_screenshot(
    ...
    logger.info(f"[ADDON UPLOAD] Valid authorization found for user: {auth.user_id}")

    # Upload screenshot using the user from the authorization
    try:
...
async def revoke_authorization(
    ...
    # Broadcast addon_authorization_revoked event to all WebSocket clients for this user
    await ws_manager.broadcast_to_session(
        user_id=user_id,
        message={
...
@router.post("/authorizations/user/{user_id}/revoke-all")
async def revoke_all_authorizations(
    ...
    """
    count = await AddonService.revoke_all_user_authorizations(db, user_id)
...
@router.get("/authorizations/user/{user_id}/list", response_model=List[ConnectedAddonInfo])
async def list_connected_addons(
    user_id: str,
    db: AsyncSession = Depends(get_db)
):
    """
    List all connected addons for a user

    Returns only active authorizations with access tokens.
    Used by web UI for addon management.
    """
    authorizations = await AddonService.get_user_authorizations(
        db=db,
        user_id=user_id,
        active_only=True
    )
