...
async def exchange_oauth_code(
    ...
    Requires X-User-ID header with current user ID.
...
async def get_key_status(
    ...
    """
    Get OpenRouter API key status for current user
    ...
    Returns:
    - **has_key**: Whether a key is stored for this user
    ...
    try:
        service = OpenRouterOAuthService(db)
        status_info = await service.get_key_status(x_user_id)
...
async def revoke_key(
    ...
    """
    Revoke (disconnect) OpenRouter API key for current user
    ...
    try:
        service = OpenRouterOAuthService(db)
        success = await service.revoke_key(x_user_id)
