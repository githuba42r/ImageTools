...
class User(Base):
    """
    Represents a persistent user identity.

    For authenticated users (Authelia / nginx basic-auth) the UUID is generated
    on first login and looked up by username on every subsequent request.

    For anonymous (unauthenticated) deployments a single well-known UUID
    (ANONYMOUS_USER_ID) is used so all browsers share one image store.
    """
    __tablename__ = "users"

    id = Column(String, primary_key=True, index=True)  # UUID (stable, never changes)
    username = Column(String, nullable=True, index=True)   # Remote-User from Authelia / basic-auth
...
class OpenRouterKey(Base):
    """
    Store OpenRouter API keys per user.
    Keys are encrypted at rest and associated with a user identity.
    """
...
class UserSettings(Base):
    """
    Store user settings (selected AI model, preferences) per user.
    """
    __tablename__ = "user_settings"

    id = Column(String, primary_key=True, index=True)
    user_id = Column(String, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True, unique=True)
    selected_model_id = Column(String, nullable=True)  # OpenRouter model ID
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class MobileAppPairing(Base):
    """
    Store mobile app pairings/links to users.
    ...
    device_owner = Column(String, nullable=True)
    ...
    long_term_secret = Column(String, nullable=True, unique=True, index=True)
    long_term_expires_at = Column(DateTime(timezone=True), nullable=True)

    # Refresh capability (180 days)
    refresh_secret = Column(String, nullable=True, unique=True, index=True)
...
class CompressionProfile(Base):
    """
    Store custom compression profiles per user.
    System default profiles have user_id = NULL (system_default = True).
    """
    __tablename__ = "compression_profiles"

    id = Column(String, primary_key=True, index=True)  # UUID
    user_id = Column(String, ForeignKey("users.id", ondelete="CASCADE"), nullable=True, index=True)  # NULL for system defaults
    ...
    format = Column(String, nullable=False)
    retain_aspect_ratio = Column(Boolean, default=True)
...
class BrowserAddonAuthorization(Base):
    ...
    addon_identifier = Column(String, nullable=True)
