...
const BUILD_DATE = '2026-02-15T04:07:23.181Z';  // Will be updated by build script
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  captureVisibleBtn.addEventListener('click', async () => {
    ...
    await browser.runtime.sendMessage({ action: 'captureVisible', tabId: tab.id });
    window.close();
  });
  
  captureFullBtn.addEventListener('click', async () => {
    const [tab] = await browser.tabs.query({ active: true, currentWindow: true });
    await browser.runtime.sendMessage({ action: 'captureFull', tabId: tab.id });
    window.close();
  });
  
  captureSelectionBtn.addEventListener('click', async () => {
    const [tab] = await browser.tabs.query({ active: true, currentWindow: true });
    await browser.runtime.sendMessage({ action: 'captureSelection', tabId: tab.id });
    window.close();
  });
